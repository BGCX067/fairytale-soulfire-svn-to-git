...
public class FillLayout extends LayoutManager {
	...
	public void doLayout(Container container, List<IWidget> content) {
		Dimension contentSize = new Dimension(container.getAppearance().getContentWidth(), container.getAppearance().getContentHeight());
		for (IWidget c : content)
		{
			c.getSize().setSize(contentSize.getWidth(), contentSize.getHeight());
			c.getPosition().setXY(0, 0);
		}
	}
