...
public class RowExLayout extends LayoutManager {
	...
	private void placeWidgets(Container container, List<IWidget> widgets) {
		...
		if (horizontal) {
			for (IWidget widget : widgets) {
				...
				widget.setY(widgetData.getAlign().alignY(container.getAppearance().getContentHeight(), widget.getSize().getHeight()));
			...
			for (int i = (widgets.size() - 1); i >= 0; i--) {
				...
				widget.setX(widgetData.getAlign().alignX(container.getAppearance().getContentWidth(), widget.getSize().getWidth()));
	...
	private void computeIndividualSpace(Container container, List<IWidget> widgets, int spaceLeft) {
		...
		for (IWidget widget : widgets) {
			...
			if (widgetData.isGrab()) {
				if (horizontal) {
					this.setValidSize(widget, (int) ((sizeUnified * widgetData.getWeight())), widget
